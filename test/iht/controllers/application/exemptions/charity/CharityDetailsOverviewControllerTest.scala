...
class CharityDetailsOverviewControllerTest extends ApplicationControllerTest {
  ...
  "Charity details overview controller" must {
    ...
    "load the page with charity details where one charity stored" in {

      val firstCharity = iht.testhelpers.CommonBuilder.charity
      val firstCharityName = iht.testhelpers.CommonBuilder.charity.name.get
      val firstCharityNumber = iht.testhelpers.CommonBuilder.charity.number.get
      ...
      status(result) should be(OK)
      contentAsString(result) should include(firstCharityName)
      contentAsString(result) should include(firstCharityNumber)
    }

    "load the page with details second charity details where two charities stored" in {

      val firstCharity = iht.testhelpers.CommonBuilder.charity
      val secondCharity = iht.testhelpers.CommonBuilder.charity copy (
        id = Some("2"), name = Some("Charity2"),
        number = Some("987654"), totalValue = Some(BigDecimal(5000)))
      val secondCharityName = secondCharity.name.get
      val secondCharityNumber = secondCharity.number.get
      val secondCharityValue = CommonHelper.numberWithCommas(secondCharity.totalValue.get)
