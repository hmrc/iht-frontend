...
class ApplyingForProbateControllerTest
  ...
  "ApplyingForProbateController" must {
    ...
    "load when visited for the first time and show a Continue link" in {
      ...
      contentResult must include(messagesApi("page.iht.registration.applicant.applyingForProbate", DeceasedInfoHelper.getDeceasedNameOrDefaultString(regdDetailsWithDeceasedDetails)))
      contentResult must include(messagesApi("page.iht.registration.applicant.applyingForProbate.p1"))
      contentResult must include(messagesApi("page.iht.registration.applicant.applyingForProbate.p2", DeceasedInfoHelper.getDeceasedNameOrDefaultString(regdDetailsWithDeceasedDetails)))
