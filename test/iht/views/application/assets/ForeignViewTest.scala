...
class ForeignViewTest  extends ShareableElementInputViewBehaviour[BasicEstateElement] {
  ...
  lazy val foreignView: foreign = app.injector.instanceOf[foreign]

  override def form:Form[BasicEstateElement] = foreignForm
  override def formToView:Form[BasicEstateElement] => Appendable = form => foreignView(form, regDetails)
