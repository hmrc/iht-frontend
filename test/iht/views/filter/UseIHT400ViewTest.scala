...
class UseIHT400ViewTest extends UnitSpec with FakeIhtApp with HtmlSpec {
  ...
  "Use Paper Form view, when rendering for an estate exceeding £1 million" must {
    ...
    "display the correct title" in {
      ...
      title.text should be(Messages("iht.useIHT400PaperForm"))
    }
    ...
    "display the correct browser title" in {
      ...
      browserTitle.text should include(Messages("iht.useIHT400PaperForm"))
    }

    "contain content advising why you must use a paper IHT-400 form" in {
      val result = use_iht400()
      val content = contentAsString(result)
      content should include(Messages("page.iht.filter.paperform.million.p1"))
      content should include(Messages("page.iht.filter.paperform.million.p2"))
    }
    ...
    "contain content with link to IHT-400 paper form" in {
      ...
      linkElement.text should be(Messages("page.iht.filter.paperform.iht400.link.text"))
    }
    ...
    "contain a link with the button class with the text 'Exit to IHT-400 paper form'" in {
      ...
      button.text() should be(Messages("page.iht.filter.paperform.million.exit"))
    }
    ...
    "contain a row showing the user's answer to the previous estimate question when given the under 32500 parameter" in {
      ...
      row.text() should include(Messages("page.iht.filter.estimate.choice.over"))
    }
