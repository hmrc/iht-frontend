...
@Singleton
class EstatePassedToDeceasedOrCharityController @Inject() (implicit val messagesApi: MessagesApi, val ihtProperties: IhtProperties) extends EstateController {
            ...
            applicationDetails match {
              case Some(appDetails) => {
                ...
                Ok(iht.views.html.application.tnrb.estate_passed_to_deceased_or_charity(
                  filledForm,
                  deceasedName,
                  CommonHelper.addFragmentIdentifier(cancelUrl, Some(ihtProperties.TnrbEstatePassedToDeceasedID))
                ))
              }
    ...
    for {
      ...
      savedApplicationDetails.fold[Result] {
        ...
        updatedAppDetailsWithKickOutReason.kickoutReason match {
          ...
          case _ => TnrbHelper.successfulTnrbRedirect(updatedAppDetailsWithKickOutReason, Some(ihtProperties.TnrbEstatePassedToDeceasedID))
        }
      }
    }
  }
