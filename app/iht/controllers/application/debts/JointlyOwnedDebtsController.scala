...
class JointlyOwnedDebtsController @Inject()(val messagesApi: MessagesApi, val ihtProperties: IhtProperties, val applicationForms: ApplicationForms) extends EstateController {
  ...
  def onSubmit = authorisedForIht {
    implicit user => implicit request => {
      ...
      estateElementOnSubmit[BasicEstateElementLiabilities](
        ...
        CommonHelper.addFragmentIdentifier(debtsRedirectLocation, Some(ihtProperties.DebtsOwedJointlyID))
      )
    }
  }
}
