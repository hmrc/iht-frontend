...
object IhtFormValidator extends FormValidator {
  def validateOptionalYesNoQuestion = new Formatter[Option[Boolean]] {
    override def bind(key: String, data: Map[String, String]) = {
      val errors = new scala.collection.mutable.ListBuffer[FormError]()
      val value = toBoolean(data.get(key))

      if (!value.isDefined) {
        errors += FormError(key, "error.selectAnswer")
      }
      if (errors.isEmpty) {
        try {
          data.get(key) match {
  ...
  def validatePrivatePensions(value: String, sharedValue: String, radioTarget: String) = new Formatter[Option[Boolean]] {
    override def bind(key: String, data: Map[String, String]) = {
      val errors = new scala.collection.mutable.ListBuffer[FormError]()
      val isChanged = toBoolean(data.get(key))
      val pensionValue = toCurrency(data.get(value))
      val pensionSharedValue = toCurrency(data.get(sharedValue))

      if (pensionValue != "" || pensionSharedValue != "") {
        if (!isChanged.isDefined) {
          errors += FormError(radioTarget, "error.value.blank")
        }
      }
      if (errors.isEmpty) {
        try {
  ...
  def validateInsurancePolicies(value: String,
                                sharedValue: String,
                                annuitiesBought: String,
                                insurancePremiumsPayedForSomeoneElse: String) = new Formatter[Option[Boolean]] {
    override def bind(key: String, data: Map[String, String]) = {
      val errors = new scala.collection.mutable.ListBuffer[FormError]()
      val isAnnuitiesBought = toBoolean(data.get(annuitiesBought))
      val isInsurancePremiumsPayedForSomeoneElse = toBoolean(data.get(insurancePremiumsPayedForSomeoneElse))
      val insuranceValue = toCurrency(data.get(value))
      val insuranceSharedValue = toCurrency(data.get(sharedValue))

      if (insuranceValue != "" || insuranceSharedValue != "") {
        if (!isAnnuitiesBought.isDefined) {
          errors += FormError(annuitiesBought, "error.value.blank")
        }
      }

      performValidateInsurancePoliciesInsPaidForSoElse(isAnnuitiesBought,
        isInsurancePremiumsPayedForSomeoneElse, insurancePremiumsPayedForSomeoneElse,
        errors)
  ...
  private def performValidateInsurancePoliciesInsPaidForSoElse(isAnnuitiesBought: Option[Boolean],
                                                               isInsurancePremiumsPayedForSomeoneElse: Option[Boolean],
                                                               insurancePremiumsPayedForSomeoneElse: String,
                                                               errors: scala.collection.mutable.ListBuffer[FormError]) = {
    isAnnuitiesBought.foreach { annuitiesBought =>
      if (!annuitiesBought && !isInsurancePremiumsPayedForSomeoneElse.isDefined) {
        errors += FormError(insurancePremiumsPayedForSomeoneElse, "error.value.blank")
      }
    }
  }

  def validateAssetsHeldInTrust(value: String, moreThanOne: String) = new Formatter[Option[Boolean]] {
    override def bind(key: String, data: Map[String, String]) = {
      val errors = new scala.collection.mutable.ListBuffer[FormError]()
      val isMoreThanOne = toBoolean(data.get(moreThanOne))
      val heldInTrustValue = toCurrency(data.get(value))

      if (heldInTrustValue != "") {
        if (!isMoreThanOne.isDefined) {
          errors += FormError(moreThanOne, "error.value.blank")
        }
      }
      if (errors.isEmpty) {
        try {
